...
import { fadeUp, springs, staggerContainer } from "@/lib/animations";
...
export function QuizIntro({ onStart }: { onStart: () => void }) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      exit={{ opacity: 0, scale: 1.1, filter: "blur(12px)", transition: { duration: 0.35 } }}
    >
      {/* Arhetipurile se rotesc pe o orbită în jurul moriștii */}
      <motion.div variants={fadeUp} className="relative size-56 sm:size-72">
        ...
        <Morisca palette="archetypes" className="absolute inset-[30%] animate-spin-slow" />
        <motion.div
        ...
        >
          {ARCHETYPE_IDS.map((id, i) => {
            ...
            return (
              <motion.span
                key={id}
                className="absolute -translate-1/2 text-4xl sm:text-5xl"
      ...
      >
        Ce tip de <span className="text-gradient">BESTan</span> ești?
      </motion.h1>
